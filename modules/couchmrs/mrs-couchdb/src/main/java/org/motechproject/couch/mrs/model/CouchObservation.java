...
public class CouchObservation<T> implements Observation<T> {
    ...
    public void addDependantObservation(Observation mrsObservation) {

        if (this.dependantObservations == null) {
            dependantObservations = new HashSet<>();
        }

        List<? extends Observation> existingObservationList = Lambda.filter(having(on(CouchObservation.class).getConceptName(), is(equalTo(mrsObservation.getConceptName()))), dependantObservations);
